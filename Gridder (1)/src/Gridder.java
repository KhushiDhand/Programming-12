...
public class Gridder extends javax.swing.JFrame 
    ...
    private void ProblemSevenActionPerformed(ActionEvent evt) {//GEN-FIRST:event_ProblemSevenActionPerformed
        // TODO add your handling code here:
        ...
        for (int row = 1; row < 99; row++) {
            for (int col = 1; col < 99; col++) {
                int counter = 0;
                if (grid[col][row - 1] == 1) {//check above temp
                    counter = counter + 1;
                }
    ...
    private void ProblemEightActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ProblemEightActionPerformed
        ...
        for (int row = 1; row < 99; row++) {
            for (int col = 1; col < 99; col++) {
                int counter = 0;
                if (grid[col - 1][row] == 0) { //check to the left
                    counter = counter + 1;
                    grid[col][row] = 1;
                }
                if (grid[col + 1][row] == 0) {//check to the right
                    grid[col][row] = 0;
                }
                if (counter >= 2) {
                    temp[col+1][row] = 1;
